        ...
        <div className="w-96 shrink-0 border-l border-[rgba(184,115,51,0.2)] bg-[#1C1917] flex flex-col" style={{ height: '100vh', position: 'sticky', top: 0 }}>
          ...
          <div className="shrink-0 px-3 py-2.5 border-b border-[rgba(184,115,51,0.15)]">
            <div className="grid grid-cols-3 gap-1.5">
              <button
                onClick={() => {
                  if (accountType === 'free') { setShowUpgradePrompt(true); return }
                  setShowUpgradePrompt(false)
                  setAgentMode('auto')
                }}
                className={`relative flex flex-col items-start px-2.5 py-2 rounded-lg border text-left transition-colors ${
                  agentMode === 'auto' && !showUpgradePrompt
                    ? 'bg-[#B87333] border-[#B87333] text-[#F7F3EC]'
                    : 'border-[rgba(184,115,51,0.2)] text-[#A89070] hover:bg-[#231F1B]'
                }`}
              >
                {accountType === 'free' && <Lock className="absolute top-1.5 right-1.5 w-2.5 h-2.5 opacity-50" />}
                <Wand2 className="w-3.5 h-3.5 mb-1" />
                <span className="text-xs font-semibold leading-none">Auto</span>
                <span className={`text-[10px] leading-tight mt-1 ${agentMode === 'auto' && !showUpgradePrompt ? 'text-[#F7F3EC]/70' : 'text-[#7A6555]'}`}>Fix everything</span>
              </button>
              <button
                onClick={() => {
                  if (accountType === 'free') { setShowUpgradePrompt(true); return }
                ...
                className={`relative flex flex-col items-start px-2.5 py-2 rounded-lg border text-left transition-colors ${
                  agentMode === 'assist' && !showUpgradePrompt
              ...
              >
                ...
                <span className={`text-[10px] leading-tight mt-1 ${agentMode === 'assist' && !showUpgradePrompt ? 'text-[#F7F3EC]/70' : 'text-[#7A6555]'}`}>Fix one thing</span>
              </button>
              <button
                onClick={() => {
                  setShowUpgradePrompt(false)
                ...
                className={`relative flex flex-col items-start px-2.5 py-2 rounded-lg border text-left transition-colors ${
                  agentMode === 'review' && !showUpgradePrompt
                    ? 'bg-[#231F1B] border-[rgba(184,115,51,0.5)] text-[#F7F3EC]'
                    : 'border-[rgba(184,115,51,0.2)] text-[#A89070] hover:bg-[#231F1B]'
                }`}
              >
                ...
                <span className={`text-[10px] leading-tight mt-1 ${agentMode === 'review' && !showUpgradePrompt ? 'text-[#F7F3EC]/70' : 'text-[#7A6555]'}`}>Second opinion</span>
              </button>
            </div>
          </div>
          ...
          {showUpgradePrompt ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              ...
              <h3 className="font-semibold text-[#F7F3EC] text-sm mb-2">Paid feature</h3>
              <p className="text-sm text-[#A89070] mb-5 leading-relaxed">
                Auto and Assist modes are available on paid plans. Upgrade for direct rewrites and full-article automation.
              </p>
              <Link
                href="/pricing"
