...
function computeGEO(content: string) {
  let score = 0
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const breakdown: Record<string, any> = {}
...
function computeAEO(content: string) {
  let score = 0
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const breakdown: Record<string, any> = {}
...
function buildRankingPrediction(difficulty: number | null, seoScore: number) {
...
function buildTrafficPrediction(searchVolume: number | null) {
